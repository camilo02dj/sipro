...
class ModeloVentas
{
    static public function mdlMostrarVentas($tabla, $fechaInicial, $fechaFinal,) {
        ...
        $baseSQL = "SELECT t.co, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo";
        ...
        if ($fechaInicial != null && $fechaInicial != $fechaFinal) {
    ...
    static public function mdlDetalleVentas($tabla, $item, $valor, $fechaInicial, $fechaFinal)
    {
        $columnasPermitidas = ['item_proveedor', 'item', 'centro_operacion'];
        if (!in_array($item, $columnasPermitidas)) {
            throw new Exception("El nombre del item '$item' no es válido");
