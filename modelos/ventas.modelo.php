...
//////////////////////////////////////////////////////
static public function mdlMostrarVentasT($tabla, $fechaInicial, $fechaFinal)
{

    if ($fechaInicial == null) {

        $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item_proveedor, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo GROUP BY t.desc_item");
        $stmt->execute();
        return $stmt->fetchAll();
    } else if ($fechaInicial == $fechaFinal) {

        $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item_proveedor, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.fecha like '%$fechaFinal%' GROUP BY t.desc_item");
        ...
        if ($fechaFinalMasUno == $fechaActualMasUno) {
            $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item_proveedor, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.fecha BETWEEN :fechaInicial AND :fechaFinalMasUno GROUP BY t.desc_item");
            $stmt->bindParam(":fechaInicial", $fechaInicial, PDO::PARAM_STR);
            $stmt->bindParam(":fechaFinalMasUno", $fechaFinalMasUno, PDO::PARAM_STR);
          
            $stmt->execute(); // Asegúrate de ejecutar la consulta dentro de esta condición
        } else {
            $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item, t.item_proveedor, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.fecha BETWEEN :fechaInicial AND :fechaFinal GROUP BY t.desc_item");
            $stmt->bindParam(":fechaInicial", $fechaInicial, PDO::PARAM_STR);
            $stmt->bindParam(":fechaFinal", $fechaFinal, PDO::PARAM_STR);
...
static public function mdlDetalleVentasT($tabla, $item, $fechaInicial, $fechaFinal)
{
    if ($fechaInicial == null) {
        // Corrección de la consulta para usar placeholders adecuadamente.
        $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.item = :item GROUP BY t.co");
       
        $stmt->bindParam(":item", $item, PDO::PARAM_STR);
    } else if ($fechaInicial == $fechaFinal) {
        // Uso de LIKE con los porcentajes directamente en la variable.
        $fechaConFormato = "%{$fechaFinal}%";
        $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.item = :item AND t.fecha LIKE :fecha GROUP BY t.co");
        $stmt->bindParam(":fecha", $fechaConFormato, PDO::PARAM_STR);
        ...
        if ($fechaFinalMasUno == $fechaActualMasUno) {
            $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.item = :item AND t.fecha BETWEEN :fechaInicial AND :fechaFinalMasUno GROUP BY t.co");
            $stmt->bindParam(":fechaFinalMasUno", $fechaFinalMasUno, PDO::PARAM_STR);
        } else {
            $stmt = Conexion::conectar()->prepare("SELECT t.co, t.item, t.fecha, t.desc_item, c.centro_operacion, SUM(t.cantidad) AS totalVendido FROM $tabla AS t INNER JOIN centro_operacion AS c ON t.co = c.codigo WHERE  t.item = :item AND t.fecha BETWEEN :fechaInicial AND :fechaFinal GROUP BY t.co");
        }
